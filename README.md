# 🤖 AI Agent Playground — Google AI Studio ReAct Agent
...
A fully functional **ReAct Agent** built with **LangChain + Streamlit** that uses **Google AI Studio (Gemini models)** as the brain, with your choice of **3 search tools** and **2 weather APIs**.

Users enter their own Google AI Studio API key, load available models dynamically, select one, and chat. No keys are hardcoded. All keys stay in the browser session only.
...
|----------|--------|------------|
| **Google AI Studio** | ALL Gemini models (gemini-2.0-flash, gemini-1.5-flash, gemini-1.5-pro, etc.) | aistudio.google.com |
...
3. **Enter Google AI Studio API Key** and click **Load Models**
4. **Choose a Gemini model** from the dropdown — ALL models are shown
5. **Ask away!**

> **Note:** DuckDuckGo requires no API key. If you choose it, you only need the Google AI Studio and weather keys.
...
git commit -m 'AI Agent Playground — Google AI Studio ReAct agent'
...
Weather Tool Selection (WeatherAPI / OpenWeatherMap)
    ↓
Dynamic Model Fetch → Model Selection (ALL Gemini models)
    ↓
...
- **Dynamic model loading.** The app fetches live model lists from Google AI Studio's API.
- **ALL models shown.** No filtering — you see every model Google offers, including gemini-2.0-flash.
