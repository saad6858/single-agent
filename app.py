...
if st.session_state.show_instructions:
    with st.expander("📖 How to Use", expanded=True):
        ...
        🔍 <b>Web Search</b> — SerpAPI (100 searches/mo free) | DuckDuckGo (completely free) | Tavily (AI search)<br>
        🌤️ <b>Weather Lookup</b> — WeatherAPI (1M calls/mo free) | OpenWeatherMap (1K calls/day free)<br>
            ...
            with st.spinner("Fetching models..."):
                try:
                    if provider == "Groq":
                        ...
                        # Try DashScope API first, fallback to hardcoded list
                        try:
                            resp = requests.get(
            ...
            if current_time - st.session_state.last_query_time < 3:
                ...
                def build_agent(provider: str, model: str, provider_key: str, search_tool_choice: str, weather_tool_choice: str, serp_key: str, tavily_key: str, weather_key: str, owm_key: str):
                    ...
                    if provider == "Groq":
                        ...
                        llm = ChatTongyi(model=model, dashscope_api_key=provider_key, temperature=0)
                    else:
