...
def friendly_error(err: Exception, search_tool: str = "", weather_tool: str = "") -> str:
    ...
    if "unsupported format string" in s and "timedelta" in s:
        return "This model does not support tool calling. Try a different model like llama-3.1-8b-instant or mixtral-8x7b."
...
<div class="hero-box">
    ...
    <div class="hero-subtitle">ReAct Agent powered by Groq — all models available, you choose the brain</div>
...
if st.session_state.show_instructions:
    with st.expander("📖 How to Use", expanded=True):
        ...
        3. Paste your Groq API key and click <b>Load Models</b><br>
        4. Choose a model from the dropdown — ALL models are shown, no filter<br>
...
with col1:
    ...
    if st.button("🔍 Load Groq Models", key="load_groq"):
            ...
            with st.spinner("Fetching models..."):
                try:
                    ...
                    if resp.status_code == 200:
                        data = resp.json()
                        # Show ALL models — zero filter, zero renaming
                        models = [m["id"] for m in data.get("data", [])]
                        st.session_state.available_models = models
                        st.session_state.provider_key = api_key_input
                        st.session_state.provider_confirmed = True
                        st.session_state.build_error = None
                        st.success(f"✅ Loaded {len(st.session_state.available_models)} Groq models")
                    else:
...
if not st.session_state.provider_confirmed:
    missing.append("Groq API Key (enter key → Load Models)")
...
if is_ready:
    ...
    <div style="display:flex;align-items:center;gap:0.7rem;margin-bottom:0.8rem;flex-wrap:wrap">
        ...
        <span style="color:#94a3b8;font-size:0.85rem">Provider: <b style="color:#e2e8f0">Groq</b></span>
...
<div style="text-align:center;color:#64748b;font-size:0.78rem;margin-top:1.5rem">
    ...
    🤖 Built with LangChain + Groq + Streamlit | BYOK | Free tier friendly
</div>
""", unsafe_allow_html=True)
