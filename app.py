...
def friendly_error(err: Exception, search_tool: str = "", weather_tool: str = "") -> str:
    ...
    if any(x in s for x in ["unsupported format string", "datetime.timedelta", "timedelta.format"]):
...
if st.session_state.show_instructions:
    with st.expander("📖 How to Use", expanded=True):
        ...
        • <b>Groq</b> — llama, mixtral, gemma, qwen families (avoid gpt-oss, it does not support ReAct)<br>
        • <b>Google AI Studio</b> — gemini-1.5-flash, gemini-1.5-pro (avoid embedding models)<br>
        • <b>Alibaba Qwen</b> — qwen-turbo, qwen-plus, qwen-max (avoid embedding models)<br><br>
                ...
                try:
                    if provider == "Groq":
                        ...
                        if resp.status_code == 200:
                            ...
                            chat_models = [m for m in models if any(x in m.lower() for x in ["llama", "mixtral", "gemma", "qwen"])]
