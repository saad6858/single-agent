...
def friendly_error(err: Exception) -> str:
    ...
    if any(x in combined for x in [
        "unsupported format string", "datetime.timedelta", "timedelta.format"
    ]):
        return "Internal LLM config error. Please refresh the page and try again."
    if "deadline exceeded" in combined:
        return "The LLM took too long to respond. Try a simpler question or check your connection."
    if any(x in combined for x in ["401", "unauthorized", "invalid api key"]):
        return "API key is invalid or expired. Please check your key and try again."
    if any(x in combined for x in ["403", "forbidden", "permission denied"]):
        return "API access denied. Your key may not have permission for this model."
    if any(x in combined for x in ["429", "rate limit", "too many requests", "quota exceeded"]):
        return "Rate limit hit. Too many requests. Wait 30 seconds and try again."
    if any(x in combined for x in ["timeout", "timed out", "connecttimeout"]):
        return "Request timed out. The server or tool took too long. Try a simpler question."
    if any(x in combined for x in ["connection error", "connection refused", "name resolution", "dns", "network"]):
        return "Network error. Please check your internet connection and try again."
    if "model not found" in combined or "not found" in combined:
        return "The selected model is not available. Try loading models again or pick a different one."

    # Tool errors
    if "duckduckgo" in combined or "ddg" in combined:
        return "DuckDuckGo search failed (rate limit or blocked). Try again in a few seconds or switch to SerpAPI/Tavily."
    if "serpapi" in combined or "serp_api" in combined:
    ...
    if "weatherapi" in combined or "weather api" in combined:
        return "WeatherAPI failed. Check your key or the city name. Try OpenWeatherMap as alternative."
    if "openweathermap" in combined or "open weather" in combined:
...
<div class="hero-box">
    ...
    <div class="hero-subtitle">ReAct Agent powered by Groq, Google AI Studio, or Alibaba Qwen — you choose the brain</div>
...
if st.session_state.show_instructions:
    with st.expander("📖 How to Use", expanded=True):
        ...
        1. Select your LLM provider (Groq, Google AI Studio, or Alibaba Qwen)<br>
...
# --- LLM Provider ---
provider = st.radio("Select LLM Provider", ["Groq", "Google AI Studio", "Alibaba Qwen"], horizontal=True, key="provider_radio")
                    ...
                    if provider == "Groq":
                        ...
                        try:
                            ...
                            if resp.status_code == 200:
                                ...
                                qwen_models = [m for m in models if m and "qwen" in m.lower()]
            ...
            if current_time - st.session_state.last_query_time < 3:
                ...
                def build_agent(provider: str, model: str, provider_key: str, search_tool_choice: str, weather_tool_choice: str, serp_key: str, tavily_key: str, weather_key: str, owm_key: str):
                    try:
                        ...
                        raise RuntimeError(f"Failed to initialize LLM: {friendly_error(e)}") from e
                    ...
                    try:
                        ...
                        raise RuntimeError(f"Search tool failed: {friendly_error(e)}") from e
                    ...
                    try:
                        ...
                        raise RuntimeError(f"Weather tool failed: {friendly_error(e)}") from e
...
Thought:{agent_scratchpad}"""
                    ...
                    try:
                        ...
                        raise RuntimeError(f"Agent assembly failed: {friendly_error(e)}") from e
