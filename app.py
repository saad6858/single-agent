...
def friendly_error(err: Exception, search_tool: str = "", weather_tool: str = "") -> str:
    ...
    if "unsupported format string" in s and "timedelta" in s:
        return "This model does not support tool calling. Try a different model like gemini-1.5-flash-latest."
...
<div class="hero-box">
    ...
    <div class="hero-subtitle">ReAct Agent powered by Google AI Studio — Gemini models only</div>
...
if st.session_state.show_instructions:
    with st.expander("📖 How to Use", expanded=True):
        ...
        1. Select your Search tool (SerpAPI, DuckDuckGo, or Tavily)<br>
        2. Select your Weather tool (WeatherAPI or OpenWeatherMap)<br>
        3. Paste your Google AI Studio API key and click <b>Load Models</b><br>
        4. Choose a Gemini model from the dropdown — ALL models are shown<br>
...
with col1:
    ...
    if st.button("🔍 Load Gemini Models", key="load_google"):
            ...
            with st.spinner("Fetching models..."):
                try:
                    ...
                    if resp.status_code == 200:
                        data = resp.json()
                        # Show ALL models — no filtering. User wants to see everything including gemini-2.0-flash
                        models = [m["name"].replace("models/", "") for m in data.get("models", [])]
                        st.session_state.available_models = models
                        st.session_state.provider_key = api_key_input
                        st.session_state.provider_confirmed = True
                        st.session_state.build_error = None
                        st.success(f"✅ Loaded {len(st.session_state.available_models)} Gemini models")
                    else:
...
if not st.session_state.provider_confirmed:
    missing.append("Google AI Studio API Key (enter key → Load Models)")
...
if is_ready:
    ...
    <div style="display:flex;align-items:center;gap:0.7rem;margin-bottom:0.8rem;flex-wrap:wrap">
        ...
        <span style="color:#94a3b8;font-size:0.85rem">Provider: <b style="color:#e2e8f0">Google AI Studio</b></span>
    ...
    if user_query:
        if len(user_query) > 1000:
            ...
            if current_time - st.session_state.last_query_time < 3:
                ...
                def build_agent(model: str, provider_key: str, search_tool_choice: str, weather_tool_choice: str, serp_key: str, tavily_key: str, weather_key: str, owm_key: str):
                    try:
...
<div style="text-align:center;color:#64748b;font-size:0.78rem;margin-top:1.5rem">
    ...
    🤖 Built with LangChain + Google AI Studio + Streamlit | BYOK | Free tier friendly
</div>
""", unsafe_allow_html=True)
