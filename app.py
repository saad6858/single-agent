...
# ============================================================
# HIDE ALL STREAMLIT CLOUD CHROME — NUCLEAR CSS
# ============================================================
_HIDE_ALL_CHROME = """
<style>
...
.hero-box{text-align:center;padding:1.2rem 1rem 0.8rem 1rem;margin-bottom:0.5rem}
...
# ============================================================
# JS: CUSTOM SIDEBAR TOGGLE + CHROME NUKE + SCROLL TO TOP
# ============================================================
...
(function(){
    ...
    window.addEventListener('load', function(){
        setTimeout(function(){ pd.scrollingElement.scrollTop = 0; }, 100);
    });
    
    // === NUKE FORK / DEPLOY / SHARE / MANAGE APP ===
    function nukeChrome(){
        var selectors = [
            'a[href*="/fork"]',
            'a[title="View source"]',
        ...
        selectors.forEach(function(sel){
            pd.querySelectorAll(sel).forEach(function(el){el.remove();});
        });
        pd.querySelectorAll('a,button,div,span').forEach(function(el){
    ...
    nukeChrome();
    var obs = new MutationObserver(function(muts){nukeChrome();});
    obs.observe(pd.body, {childList:true, subtree:true});
    setInterval(nukeChrome, 300);
    
    // === CUSTOM SIDEBAR TOGGLE BUTTON ===
    if(!pd.getElementById('custom-sidebar-toggle')){
        var btn = pd.createElement('button');
        btn.id = 'custom-sidebar-toggle';
        btn.innerHTML = '📖&nbsp;&nbsp;Instructions';
        btn.style.cssText = 'position:fixed;top:14px;right:14px;z-index:99999;background:linear-gradient(135deg,#0ea5e9,#06b6d4);color:#fff;font-weight:700;border:none;border-radius:12px;padding:10px 18px;font-size:0.85rem;cursor:pointer;box-shadow:0 4px 20px rgba(14,165,233,0.35);transition:all 0.25s ease;font-family:sans-serif;letter-spacing:0.3px;';
        btn.onmouseenter = function(){this.style.transform='translateY(-2px) scale(1.03)';this.style.boxShadow='0 8px 30px rgba(14,165,233,0.5)';};
        btn.onmouseleave = function(){this.style.transform='translateY(0) scale(1)';this.style.boxShadow='0 4px 20px rgba(14,165,233,0.35)';};
        btn.onclick = function(){
            var toggle = pd.querySelector('[data-testid="stSidebarCollapsedControl"]');
